...
// Collection access function aliases for backward compatibility
export async function getSupplierCollection() {
  return await getSuppliersCollection();
}

export async function getScheduleCollection() {
  return await getFeedingScheduleCollection();
}

// Additional validation functions
export function validateScheduleData(data) {
  const required = ['time', 'cowGroup', 'foodType', 'quantity', 'unit', 'daysOfWeek'];
  ...
  // Validate cow group
  if (!Object.values(CowGroups).includes(data.cowGroup)) {
    return { 
      valid: false, 
      errors: ['cowGroup must be a valid cow group'] 
    };
  }

  // Validate food type
  if (!Object.values(FoodTypes).includes(data.foodType)) {
    return { 
      valid: false, 
      errors: ['foodType must be a valid food type'] 
    };
  }

  // Validate unit
  if (!Object.values(UnitTypes).includes(data.unit)) {
    return { 
      valid: false, 
      errors: ['unit must be a valid unit type'] 
    };
  }
  ...
  if (!Array.isArray(data.daysOfWeek) || data.daysOfWeek.length === 0) {
    return { 
      valid: false, 
      errors: ['daysOfWeek must be a non-empty array'] 
    };
  }

  const invalidDays = data.daysOfWeek.filter(day => 
  ...
  if (invalidDays.length > 0) {
    return { 
      valid: false, 
      errors: ['daysOfWeek must contain valid day numbers (0-6)'] 
    };
  }
