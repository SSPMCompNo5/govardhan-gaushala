...
export async function GET() {
  try {
    ...
    const rows = await db.collection('maintenanceLogs').find({}).sort({ date: -1 }).limit(200).toArray();
...
export async function POST(request) {
  try {
    ...
    const parsed = MaintenanceLogSchema.safeParse({ ...raw, date: raw.date ? new Date(raw.date) : undefined });
    if (!parsed.success) return NextResponse.json({ error: 'Validation failed', details: parsed.error.flatten() }, { status: 400 });
    const client = await clientPromise;
    const db = client.db(process.env.MONGODB_DB);
    await db.collection('maintenanceLogs').insertOne({ ...parsed.data, createdAt: new Date(), updatedAt: new Date() });
